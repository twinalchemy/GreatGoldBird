	...
	public class InventoryManager : ScriptableObject
	{
		...
		private void ItemsGUI ()
		{
			...
			if (selectedItem != null && items.Contains (selectedItem))
			{
				...
				if (settingsManager && settingsManager.interactionMethod != AC_InteractionMethod.ContextSensitive && settingsManager.inventoryInteractions == InventoryInteractions.Multiple && AdvGame.GetReferences ().cursorManager)
				{
					...
					if (cursorManager.cursorIcons.Count > 0)
					{
						foreach (InvInteraction interaction in selectedItem.interactions)
						{
							...
							interaction.actionList = ActionListAssetMenu.AssetGUI ("", interaction.actionList, apiPrefix + ".interactions[" + i + "].actionList");
				...
				{
					selectedItem.useActionList = ActionListAssetMenu.AssetGUI ("Use:", selectedItem.useActionList, apiPrefix + ".useActionList");
					...
					selectedItem.lookActionList = ActionListAssetMenu.AssetGUI ("Examine:", selectedItem.lookActionList, apiPrefix + ".lookActionList");
				}
				...
				if (settingsManager.CanSelectItems (false))
				{
					...
					selectedItem.unhandledActionList = ActionListAssetMenu.AssetGUI ("Unhandled use on Hotspot:", selectedItem.unhandledActionList, apiPrefix + ".unhandledActionList");
					selectedItem.unhandledCombineActionList = ActionListAssetMenu.AssetGUI("Unhandled combine:", selectedItem.unhandledCombineActionList, apiPrefix + ".unhandledCombineActionList");
				}
				...
				for (int i=0; i<selectedItem.combineActionList.Count; i++)
				{
					...
					selectedItem.combineActionList[i] = ActionListAssetMenu.AssetGUI ("", selectedItem.combineActionList[i], apiPrefix + ".combineActionList[" + i + "]");
