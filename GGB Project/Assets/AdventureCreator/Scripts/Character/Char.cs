...
namespace AC
{
  ...
  public class Char : MonoBehaviour
  {
    ...
    // Added by ScienceSense (Greg) - for dynamic footsteps
    public float walkSoundSpeed = 10f;
    public float runSoundSpeed = 3f;
    public float audioPitchShift = .1f;
    ...
    float lastPlayedSoundTime = 0f; // Modified by ScienceSense (Greg) - dynamic footsteps  
    private void PlayStandardAudio() {
      if (audioSource) {
        if (isRunning && runSounds.Length > 0) // Modified by ScienceSense (Greg) 
        {
          if (audioSource.isPlaying && (Time.time - lastPlayedSoundTime) < runSoundSpeed/* && audioSource.clip == runSound*/) {  // Modified by ScienceSense (Greg) 
            return;
          }
          ...
          runSound = runSounds[Random.Range(0, runSounds.Length)];
          audioSource.clip = runSound;
          audioSource.volume = Random.Range(1 - audioPitchShift, 1 + audioPitchShift);
          audioSource.pitch = Random.Range(1 - audioPitchShift, 1 + audioPitchShift);
          audioSource.Play();
        } else if (walkSounds.Length > 0) {
          if (audioSource.isPlaying && (Time.time - lastPlayedSoundTime) < walkSoundSpeed /* && audioSource.clip == walkSound*/) {
            return;
          }
